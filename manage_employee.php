		...
		<div class="form-group">
			...
			<select class="custom-select browser-default select2" name="position_id">
				...
				while ($row = $pos->fetch_assoc()):
					?>
					<option class="opt" value="<?php echo $row['id'] ?>" data-did="<?php echo $row['department_id'] ?>"
						<?php //echo isset($department_id) && $department_id == $row['id'] ? "" : "disabled" ?> 
						<?php echo isset($position_id) && $position_id == $row['id'] ? "selected" : "" ?>><?php echo $row['name'] ?>
					</option>
				<?php endwhile; ?>
			</select>
		</div>
